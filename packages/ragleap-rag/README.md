...
Attach arbitrary JSON metadata to a document at ingest time, then restrict retrieval to matching chunks with metadata_filter on ask() - the basis for multi-tenant isolation, date-range filtering, or any other tagging scheme. As of v0.12.8, `metadata=` is accepted by `ingest()` too (previously only `ingest_text()`, `ingest_url()`, and `ingest_image()` had it - a real, unexplained inconsistency now closed).
